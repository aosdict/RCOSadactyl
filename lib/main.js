...
text_entry.port.on("text-entered", function (text) {
  ...
  // log whatever variable the user wants to the console
  if (text.substring(0,4) === ":log"){
    // log scroll amount
    if (text.substring(5, 14).toUpperCase() === "SCROLLAMT"){
      console.log(SCROLLAMT);
    }
  }

  if (text.substring(0,4) === ":set"){
    // parse for the wanted scroll speed and sent to scroll.js
    if (text.substring(5, 14).toUpperCase() === "SCROLLAMT"){
      SCROLLAMT = parseInt(text.substring(15,text.length));
      pagemod.port.emit("set scrollamt", SCROLLAMT);
    }
  }

  // evaluate some javascript command
  if (text.substring(0,3) === ":js" 
  || text.substring(0,11) === ":javascript"){
    var firstspace = text.indexOf(" ");
    console.log(firstspace);
    command = text.substring(firstspace,text.length);
    eval(command);
  }
  // :newtab
  ...
  // :deltab
  // delete current tab
  else if (text.substring(0,7) === ":deltab") {
    // remove all tabs to the left
    curr = tabs.activeTab.index;
    if (text.substring(7,9) === " l"){
      for (var i = 0 ; i != curr ; i++){
        tabs[i].close();
      }
    }
    // remove all tabs to the right
    else if (text.substring(7,9) === " r"){
      for (var i = curr+1 ; i != tabs.length ; i++){
        tabs[i].close();
      }
    }
    // remove all tabs except active tab
    else if (text.substring(7,9) === " x"){
  ...
  // :numerical tab changing
  // :<int>
  else if (UtilIsInt(text.substring(1,text.length))){
    newtabindex = parseInt(text.substring(1,text.length));
    if (newtabindex >=0 && newtabindex <= tabs.length-1) 
      tabs[newtabindex].activate();
  }
  // :pT
  // previous tab
